class ldapauth::client (String $basedn = 'dc=domain,dc=tld', Array $servers = ['ldap:://srv1', 'ldap://srv2']) inherits ldapauth::params { 
    ...
    Exec {"libnss_ldap":
        command		=> "auth-client-config -t nss -p lac_ldap && touch /root/.lac_ldap.done",
        creates		=> "/etc/init.d/libnss-ldap",
        unless		=> ['test -f /root/.lac_ldap.done'],
    ...
    Exec {"pam_localgroup":
        command => "pam-auth-update && touch /root/.pam_localgroup.done",
        unless  => ['test -f /root/.pam_localgroup.done'],
        require => Package["libpam-modules"],
    }
